...
public:
	...
	PredictionCopy(int type, uint8_t* dest, bool dest_packed, const uint8_t* src, bool src_packed,
		optype_t opType, FN_FIELD_COMPARE func = NULL) noexcept;

	int	 TransferData(const char* operation, int entindex, datamap_t* dmap) noexcept;
private:
	optype_t opType;
	int type;
	void* dest;
