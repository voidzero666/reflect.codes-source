...
bool CNetVarManager::HookProp(const char* tableName, const char* propName, void* fun)
{
	RecvProp* recvProp = 0;
...
{
	std::string pre(crypt_str(""));
	for (int i = 0; i < depth; i++)
		pre.append(crypt_str("\t"));
	...
	for (int i = 0; i < table->m_nProps; i++)
	{
		...
		if (varName.find(crypt_str("baseclass")) == 0 || varName.find(crypt_str("0")) == 0 || varName.find(crypt_str("1")) == 0 || varName.find(crypt_str("2")) == 0)
			continue;
